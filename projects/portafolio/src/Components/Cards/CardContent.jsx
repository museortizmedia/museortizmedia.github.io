    ...
    return (
        <>
            ...
            <div className=" card-info relative flex-1 content-center text-left p-5">
                ...
                <span className={`text-[40px] material-symbols-outlined transition duration-300 ease-out hover:ease-in absolute bottom-3 right-3 ${TailwindComponents.BotonLogo}`} onClick={OnButtonClick} translate="no">{ButonLogo}</span>
            </div>
        </>
    )
}
...
const CardColContent = ({ ImageSrc, ImageAlt, Title, SubTitle, ButonLogo = ArrowIcon, OnButtonClick }) => {
    ...
    return (
        <>
            <img className="self-center rounded-[15px] h-2/3 w-full object-cover" src={ImageSrc} alt={ImageAlt} />
